...
class GetLastLessonService(webapp2.RequestHandler):

    def post(self):
        print "GetLastLessonService received body = " + self.request.body
        ...
        if (len(fwUsers) > 0):
            lessons = FWLesson.query(FWLesson.lessonName == fwUsers[0].lastLesson).fetch() 
